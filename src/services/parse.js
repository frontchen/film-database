...
let parse = {
  ...
  parse245BtListHtml: (data) => {
    ...
    let pageDom = $(".stui-page").children("li");
    ...
    let currentDom = $(".stui-page").find(".active .num");
    currentDom = currentDom ? currentDom.text() : null;
    let currentPage = 1;
    if (currentDom) {
      currentPage = currentDom ? currentDom.split("/")[0] : 1;
    }

    let totalPage = pageDom.last().find("a").attr("href");
    totalPage = totalPage.split(".")[0].split("-")[1];
    return {
      // tabs,
      body,
      totalPage,
